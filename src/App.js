    ...
    return (
      ...
      <thead>
        <tr>
          <th>#</th>
          <th>Camper Name</th>
          <th onClick={(event)=>this.pointChange(true)}>Points in 30 days {current === true && (<i className="fa fa-caret-down"></i>)}</th>
          <th onClick={(event)=>this.pointChange(false)}>All Time Points{current !== true && (<i className="fa fa-caret-down"></i>)}</th>
        </tr>
        </thead>
        <tbody>
      {current === true && (top100Days.map((row, index)=>(
          ...
          <tr key={row.username}>
            ...
            <Image circle src = {row.img} className="imgHeight" /> {row.username}
            </a></td>
            <td>{row.recent}</td>
            <td>{row.alltime}</td>
          </tr>
